<div class="p-6" wire:poll.30s>
    ...
    {{-- Search Bar --}}
    <form method="GET" action="{{ route('admin.manage-faculties') }}" data-admin-realtime-filter class="mb-6">
    <div>
        <div class="relative max-w-md">
            ...
            <input
                ...
                name="search"
                value="{{ $search }}"
                placeholder="{{ __('search_faculties') }}"
                autocomplete="off"
                class="block w-full pl-11 pr-20 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all duration-200"
            />
            @if($search)
                <button type="button" data-admin-clear-search aria-label="{{ __('clear_search') }}" class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-gray-600">
    ...
    @if ($faculties->isEmpty())
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-12 text-center">
            ...
            @if($search)
                <p class="text-sm font-semibold text-gray-700">{{ __('no_faculties_found') }}</p>
                <p class="mt-1 text-sm text-gray-500">{{ __('try_searching_with_different_keywords_2') }}</p>
            @else
                <p class="text-sm font-semibold text-gray-700">{{ __('no_faculties_yet') }}</p>
                <p class="mt-1 text-sm text-gray-500">{{ __('start_by_adding_your_first_faculty') }}</p>
                <div class="mt-4">
                        ...
                        <div>
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-gray-100 text-gray-600">
                                    ...
                                    {{ $faculty->departments->count() }} {{ __('department_3') }}
                                </span>
                            </div>
            ...
            <div class="overflow-x-auto rounded-2xl border border-gray-200 bg-white">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('key_no') }}</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('faculty_name') }}</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">{{ __('head') }}</th>
                            <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">{{ __('department_3') }}</th>
                            <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('actions_2') }}</th>
                        </tr>
                    </thead>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full">
                <div class="p-8 text-center">
                    ...
                    <h3 class="text-lg font-bold text-gray-900 mb-2">{{ __('confirm_deletion') }}</h3>
                    <p class="text-sm text-gray-500 leading-relaxed">{{ __('are_you_sure_you_want_to_delete_the_faculty') }} <span class="font-semibold text-gray-900" id="delete-faculty-name"></span> {{ __('this_action_cannot_be_undone') }}</p>
                </div>
