        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-3">
                    <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-amber-500 text-white flex items-center justify-center shadow-sm">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">{{ __('កែប្រែកម្មវិធីសិក្សា') }}</h1>
                        <p class="text-sm text-gray-500 mt-0.5">{{ __('កែប្រែព័ត៌មានសម្រាប់') }} <span class="font-semibold text-gray-700">{{ $program->name_km ?? $program->name_en }}</span></p>
                    </div>
                </div>
            </div>
                            ...
                            <select id="pathway_program_id" name="pathway_program_id"
                                ...
                                @foreach ($programs as $p)
                                    <option value="{{ $p->id }}" {{ old('pathway_program_id', $program->pathway_program_id) == $p->id ? 'selected' : '' }}>
                                        {{ $p->name_km }} ({{ $p->name_en }}) - {{ $p->duration_years }} ឆ្នាំ
                                    </option>
                                @endforeach
                            </select>
