        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 py-10">
            ...
            <div class="flex items-center justify-between mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ __('create_new_announcement') }}</h1>
                        <p class="text-gray-500 mt-1">{{ __('fill_in_the_information_below_to_create_an_announcement') }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="title_en" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('title_english') }}</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="content_km" id="content_km" rows="6" required
                                    ...
                                    placeholder="{{ __('enter_content_in_khmer') }}">{{ old('content_km') }}</textarea>
                            </div>
                            <div>
                                <label for="content_en" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('content_english') }}</label>
                                <textarea name="content_en" id="content_en" rows="6"
                                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 placeholder-gray-400 resize-none"
                                    placeholder="{{ __('enter_content_in_english') }}">{{ old('content_en') }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="target_role" id="target_role"
                                    ...
                                    <option value="all" {{ old('target_role') == 'all' ? 'selected' : '' }}>{{ __('all_2') }}</option>
                                    <option value="student" {{ old('target_role') == 'student' ? 'selected' : '' }}>{{ __('students') }}</option>
                                    <option value="professor" {{ old('target_role') == 'professor' ? 'selected' : '' }}>{{ __('teacher') }}</option>
                                    <option value="admin" {{ old('target_role') == 'admin' ? 'selected' : '' }}>{{ __('administrator') }}</option>
                                </select>
                            </div>
                            <div></div>
                        </div>
