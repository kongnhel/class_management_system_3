        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ viewMode: localStorage.getItem('room_view') || 'table' }" x-init="$watch('viewMode', v => localStorage.setItem('room_view', v))">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ __('manage_rooms') }}</h1>
                        <p class="text-gray-500 text-sm mt-0.5">{{ __('manage_and_track_rooms_in_the_system') }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <div class="inline-flex rounded-xl bg-white border border-gray-200 p-1 shadow-sm">
                        <button @click="viewMode = 'card'"
                                ...
                                class="p-2 rounded-lg transition-all duration-200" title="{{ __('card_view') }}">
                        ...
                        <button @click="viewMode = 'table'"
                                ...
                                class="p-2 rounded-lg transition-all duration-200" title="{{ __('table_view') }}">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <form method="GET" action="{{ route('admin.rooms.index') }}" data-admin-realtime-filter>
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="relative flex-1">
                            ...
                            <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('search_rooms') }}"
            ...
            @if($rooms->isEmpty())
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-12">
                    <div class="text-center max-w-sm mx-auto">
                        ...
                        <h3 class="text-lg font-bold text-gray-900 mb-2">{{ $search ? __('no_results_found') : __('no_rooms_yet') }}</h3>
                        <p class="text-gray-500 text-sm mb-6">
                            @if($search)
                                {{ __('no_room_matching_search_try_again', ['search' => $search]) }}
                            @else
                                {{ __('click_the_button_below_to_create_a_new_room') }}
                            @endif
                        </p>
                        @if(!$search)
                            <a href="{{ route('admin.rooms.create') }}"
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-100">
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('room') }}</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('location') }}</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('type') }}</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('capacity_2') }}</th>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
                        ...
                        <div class="space-y-2 mb-4">
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                <span>{{ $room->type_of_room ?? __('regular_classroom_2') }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                <svg class="h-4 w-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/>
                                </svg>
                                <span>{{ $room->capacity }} {{ __('students_2') }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                <svg class="h-4 w-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-500">
                            {{ __('showing') }} {{ $rooms->firstItem() }}-{{ $rooms->lastItem() }} {{ __('key_of') }} {{ $rooms->total() }} {{ __('results') }}
                        </p>
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl relative" onclick="event.stopPropagation()">
            <div class="text-center">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('confirm_deletion') }}</h3>
                <p class="text-gray-500 text-sm">{{ __('are_you_sure_you_want_to_delete_room') }} <strong id="delete-room-name" class="text-gray-900"></strong> {{ __('key_') }}</p>
            </div>
