        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ viewMode: localStorage.getItem('room_view') || 'table' }" x-init="$watch('viewMode', v => localStorage.setItem('room_view', v))">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ __('គ្រប់គ្រងបន្ទប់') }}</h1>
                        <p class="text-gray-500 text-sm mt-0.5">{{ __('គ្រប់គ្រង និងតាមដានបញ្ជីបន្ទប់ក្នុងប្រព័ន្ធ') }}</p>
                    </div>
                </div>
                ...
                <form method="GET" action="{{ route('admin.rooms.index') }}" data-admin-realtime-filter>
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="relative flex-1">
                            ...
                            <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('ស្វែងរកបន្ទប់...') }}"
            ...
            @if($rooms->isEmpty())
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-12">
                    <div class="text-center max-w-sm mx-auto">
                        ...
                        <h3 class="text-lg font-bold text-gray-900 mb-2">{{ $search ? __('រកមិនឃើញលទ្ធផល') : __('មិនទាន់មានបន្ទប់') }}</h3>
                        <p class="text-gray-500 text-sm mb-6">
                            @if($search)
                                {{ __('រកមិនឃើញបន្ទប់ដែលត្រូវនឹង ":search" ។ សូមព្យាយាមម្តងទៀត។', ['search' => $search]) }}
                            @else
                                {{ __('ចុចប៊ូតុងខាងក្រោមដើម្បីបង្កើតបន្ទប់ថ្មី។') }}
                            @endif
                        </p>
                        @if(!$search)
                            <a href="{{ route('admin.rooms.create') }}"
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-100">
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('បន្ទប់') }}</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('ទីតាំង') }}</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('ប្រភេទ') }}</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('សមត្ថភាពផ្ទុក') }}</th>
                ...
                <div class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-500">
                            {{ __('បង្ហាញ') }} {{ $rooms->firstItem() }}-{{ $rooms->lastItem() }} {{ __('ក្នុងចំណោម') }} {{ $rooms->total() }} {{ __('លទ្ធផល') }}
                        </p>
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl relative" onclick="event.stopPropagation()">
            <div class="text-center">
                ...
                <p class="text-gray-500 text-sm">{{ __('តើអ្នកប្រាកដថាចង់លុបបន្ទប់') }} <strong id="delete-room-name" class="text-gray-900"></strong> {{ __('មែនទេ?') }}</p>
            </div>
