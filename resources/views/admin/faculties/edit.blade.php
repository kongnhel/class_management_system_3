        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">{{ __('edit_faculty') }}</h1>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
                <form method="POST" action="{{ route('admin.update-faculty', $faculty->id) }}">
                    ...
                    {{-- Name Khmer --}}
                    <div class="mb-5">
                        <label for="name_km" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('faculty_name_khmer') }} <span class="text-red-500">*</span></label>
                        <input
                            id="name_km"
                            type="text"
                            name="name_km"
                    ...
                    {{-- Name English --}}
                    <div class="mb-5">
                        <label for="name_en" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('faculty_name_english') }} <span class="text-red-500">*</span></label>
                        <input
                            id="name_en"
                            type="text"
                            name="name_en"
                    ...
                    {{-- Dean --}}
                    <div class="mb-8">
                        <label for="dean_user_id" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('faculty_dean') }}</label>
                        <select
                            id="dean_user_id"
                            name="dean_user_id"
                        ...
                        >
                            <option value="">{{ __('select_a_dean_optional') }}</option>
                    ...
                    @if($faculty->departments->count() > 0)
                        <div class="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                <span>{{ __('this_faculty_has') }} <span class="font-semibold text-gray-900">{{ $faculty->departments->count() }}</span> {{ __('department_3') }}</span>
                            </div>
                        </div>
                    @endif
