<x-app-layout>
    <div class="min-h-screen bg-slate-50 py-8 px-4">
        <div class="max-w-xl mx-auto">
            <div class="mb-6">
                <h1 class="text-2xl font-black text-slate-900">{{ __('Attendance ID Card') }}</h1>
                <p class="text-sm text-slate-500 mt-1">{{ __('Save this card on your phone and show it to your professor when attendance is open.') }}</p>
            </div>
            ...
            @if ($card && ! $card->revoked_at)
                <div class="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 text-center">
                    ...
                    <div class="mt-5 flex gap-3">
                        ...
                        <button type="button" onclick="downloadAttendanceCard('jpg')" class="flex-1 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white hover:bg-blue-700">{{ __('Save JPG') }}</button>
                        <form method="POST" action="{{ route('student.attendance-card.revoke') }}" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full rounded-xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-600 hover:bg-rose-100">{{ __('Revoke Card') }}</button>
                        </form>
                    </div>
                </div>
            @else
                <div class="bg-white rounded-3xl shadow-xl border border-slate-200 p-6">
                    <h2 class="text-lg font-black text-slate-900">{{ __('Create your attendance card') }}</h2>
                    <p class="mt-2 text-sm leading-6 text-slate-600">{{ __('Create a private QR card once, then save it to your phone. You can revoke it and generate a new one if your phone is lost.') }}</p>
                    <form method="POST" action="{{ route('student.attendance-card.create') }}" class="mt-6">
                        @csrf
                        <button type="submit" class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-700">{{ __('Create Attendance Card') }}</button>
                    </form>
                </div>
            @endif
        </div>
