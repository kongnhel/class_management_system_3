...
<title>{{ config('app.name', 'Class Management System') }} - {{ __('auth_2fa_password') }}</title>
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 px-6 py-12">
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-extrabold text-gray-900">{{ __('auth_2fa_password') }}</h2>
                <p class="text-gray-500 mt-2 text-sm leading-relaxed">
                    {{ __('auth_2fa_has_2fa') }}<br>
                    {{ __('auth_2fa_enter_password') }}
                </p>
                @if($hint)
                    <div class="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-amber-50 rounded-xl border border-amber-200">
                        ...
                        <span class="text-sm text-amber-700">{{ __('auth_2fa_hint_label') }} {{ $hint }}</span>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
                <form method="POST" action="{{ route('phone-otp.2fa.verify') }}">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('auth_2fa_telegram_password') }}</label>
                        <div class="relative">
                            <input id="password" type="password" name="password" required
                                   ...
                                   placeholder="{{ __('auth_2fa_placeholder') }}" autofocus />
