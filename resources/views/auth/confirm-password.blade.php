...
<title>{{ config('app.name', 'Class Management System') }} - {{ __('auth_confirm_password') }}</title>
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 px-6 py-12">
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-extrabold text-gray-900">{{ __('auth_confirm_password') }}</h2>
                <p class="text-gray-500 mt-2 text-sm leading-relaxed">{{ __('auth_confirm_password_subtitle') }}</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
                <form method="POST" action="{{ route('password.confirm') }}" class="space-y-5">
                    ...
                    <button type="submit" class="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg shadow-emerald-200 hover:shadow-emerald-300 transition-all duration-200 text-sm">{{ __('auth_confirm_btn') }}</button>
                </form>
            </div>
        </div>
    </div>
