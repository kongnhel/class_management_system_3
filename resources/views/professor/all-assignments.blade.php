        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8">
                <h3 class="text-3xl font-bold text-gray-800 mb-8 flex items-center">
                    <i class="fas fa-file-alt mr-3 text-red-600"></i>{{ __('all_assignments') }}
                </h3>
                    ...
                    <table class="min-w-full leading-normal">
                        <thead>
                            <tr class="bg-gray-200 text-gray-700 uppercase text-sm font-semibold">
                                ...
                                <th class="py-3 px-4 text-left">{{ __('due_date') }}</th>
                            ...
                            @forelse ($assignments as $assignment)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-4 text-gray-600">
                                        ...
                                        @else
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{{ __('remaining') }}</span>
                                        @endif
                                    </td>
                                    <td class="py-3 px-4 text-center space-x-2">
                                        {{-- Link to view submissions for this assignment (requires a specific route) --}}
                                        <a wire:navigate href="{{ route('professor.manage-assignments', ['offering_id' => $assignment->course_offering_id]) }}" class="text-emerald-600 hover:text-emerald-800 font-semibold py-1 px-3 rounded-full text-sm transition-colors duration-200 hover:bg-emerald-100">
                                            {{ __('view_submissions') }} (0)
                                        </a>
                                        ...
                                        <form action="#" method="POST" class="inline-block" onsubmit="return confirm('{{ __('confirm_delete_this') }}');">
                                            @csrf
                                            @method('DELETE')
