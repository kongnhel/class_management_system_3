<x-app-layout>
    <x-slot name="header">
        ...
        <p class="mt-1 text-lg text-gray-500">{{ __('manage_teaching_courses') }}</p>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl p-8 lg:p-12 border border-gray-200 transition-all duration-300 transform hover:shadow-3xl">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between mb-10 pb-4 border-b border-gray-200">
                    <div>
                        ...
                        <p class="mt-2 text-lg text-gray-500">{{ __('view_manage_assigned_courses') }}</p>
                    </div>
                    {{-- <div class="mt-4 md:mt-0">
                        <a wire:navigate href="{{ route('admin.create-course') }}" class="px-8 py-3 bg-gradient-to-r from-green-500 to-teal-600 text-white font-bold rounded-full shadow-lg hover:from-green-600 hover:to-teal-700 transition duration-300 transform hover:scale-105 flex items-center space-x-2">
                ...
                <div class="mt-8">
                    @if ($courses->isEmpty())
                        <div class="bg-white p-12 rounded-3xl text-center text-gray-400 shadow-xl border border-gray-100">
                            ...
                            <p class="font-semibold text-lg">{{ __('no_courses_yet') }}</p>
                            <p class="mt-2 text-sm">{{ __('start_by_adding_first_course') }}</p>
                        </div>
                    @else
                            ...
                            @foreach ($courses as $course)
                                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 flex flex-col justify-between hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <div class="space-y-2 mb-6">
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('course_code') }}</span>: <span class="text-gray-600">{{ $course->code ?? 'N/A' }}</span></p>
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('credits') }}</span>: <span class="text-gray-600">{{ $course->credits ?? 'N/A' }}</span></p>
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('department_3') }}</span>: <span class="text-gray-600">{{ $course->department->name_km ?? 'N/A' }}</span></p>
                                    </div>
                                    <div class="flex justify-end space-x-3 mt-auto">
                                        {{-- <a href="{{ route('professor.view-courses', $course->id) }}" class="p-3 bg-gray-100 rounded-full text-emerald-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('view') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                                <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                            </svg>
                                        </a> --}}
                                        {{-- <a href="{{ route('admin.edit-course', $course->id) }}" class="p-3 bg-gray-100 rounded-full text-yellow-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('edit_2') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                                <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.delete-course', $course->id) }}" method="POST" onsubmit="return confirm('{{ __('confirm_delete_course') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-3 bg-gray-100 rounded-full text-red-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('delete_2') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
