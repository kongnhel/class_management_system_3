        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8">
                <h3 class="text-3xl font-bold text-gray-800 mb-8 flex items-center">
                    <i class="fas fa-pen-square mr-3 text-purple-600"></i>{{ __('បញ្ជីការប្រលងទាំងអស់ដែលខ្ញុំគ្រប់គ្រង') }}
                </h3>
                    ...
                    <table class="min-w-full leading-normal">
                        <thead>
                            <tr class="bg-gray-200 text-gray-700 uppercase text-sm font-semibold">
                                ...
                                <th class="py-3 px-4 text-left">{{ __('ថ្ងៃប្រលង') }}</th>
                                <th class="py-3 px-4 text-left">{{ __('រយៈពេល (នាទី)') }}</th>
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-4 text-gray-600">
                                        ...
                                        @elseif (\Carbon\Carbon::parse($exam->exam_date)->isToday())
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ __('ថ្ងៃនេះ') }}</span>
                                        @else
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">{{ __('ជិតដល់') }}</span>
                                        @endif
                                    </td>
                                    <td class="py-3 px-4 text-center space-x-2">
                                        ...
                                        <form action="#" method="POST" class="inline-block" onsubmit="return confirm('{{ __('តើអ្នកពិតជាចង់លុបការប្រលងនេះមែនទេ?') }}');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-4 px-6 text-center text-gray-500">
                                        {{ __('មិនទាន់មានការប្រលងណាមួយត្រូវបានកំណត់នៅឡើយទេ។') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
