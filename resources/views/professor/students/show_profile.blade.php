    ...
    <div class="py-6 bg-gray-50 min-h-screen">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 space-y-3">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                ...
                @php
                    $enrollment = $student->studentProgramEnrollments->where('status', 'active')->first();
                    $fields = [
                        ['icon' => 'M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222', 'label' => __('ឯកទេសសិក្សា'), 'value' => $student->program->name_km ?? $student->program->name_en ?? 'N/A'],
                        ['icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', 'label' => __('មហាវិទ្យាល័យ'), 'value' => $enrollment?->program?->department?->faculty?->name_km ?? 'N/A'],
                        ['icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', 'label' => __('ជំនាញ'), 'value' => $enrollment?->program?->department?->name_km ?? 'N/A'],
