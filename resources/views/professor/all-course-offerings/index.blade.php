<x-app-layout>
    <x-slot name="header">
        ...
        <p class="mt-1 text-lg text-gray-500">{{ __('browse_manage_all_offerings') }}</p>
    </x-slot>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between mb-10 pb-4 border-b border-gray-200">
                    <div>
                        <h2 class="font-extrabold text-4xl text-gray-900 leading-tight">
                            {{ __('all_course_offerings') }}
                        </h2>
                        <p class="mt-2 text-lg text-gray-500">{{ __('browse_all_course_offerings') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-8">
                    @if ($courseOfferings->isEmpty())
                        <div class="bg-white p-12 rounded-3xl text-center text-gray-400 shadow-xl border border-gray-100">
                            ...
                            <p class="font-semibold text-lg">{{ __('no_course_offerings_yet') }}</p>
                            <p class="mt-2 text-sm">{{ __('start_by_creating_offerings') }}</p>
                        </div>
                    @else
                            ...
                            @foreach ($courseOfferings as $offering)
                                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 flex flex-col justify-between hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <div class="space-y-2 mb-6">
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('academic_year') }}</span>: <span class="text-gray-600">{{ $offering->academic_year ?? 'N/A' }}</span></p>
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('semester') }}</span>: <span class="text-gray-600">{{ $offering->semester ?? 'N/A' }}</span></p>
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('section') }}</span>: <span class="text-gray-600">{{ $offering->section ?? 'N/A' }}</span></p>
                                        <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('teacher') }}</span>: <span class="text-gray-600">{{ $offering->lecturer?->name ?? 'N/A' }}</span></p>
                                    </div>
                                    {{-- <div class="flex justify-end space-x-3 mt-auto">
                                        <a wire:navigate href="{{ route('professor.view-all-course-offerings', $offering->id) }}" class="p-3 bg-gray-100 rounded-full text-emerald-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('view') }}">
