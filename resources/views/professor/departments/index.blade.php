<x-app-layout>
    <x-slot name="header">
        ...
        <p class="mt-1 text-lg text-gray-500">{{ __('បញ្ជីឈ្មោះដេប៉ាតឺម៉ង់ទាំងអស់នៅក្នុងប្រព័ន្ធ') }}</p>
    </x-slot>
                ...
                <div class="mt-8">
                    @if ($departments->isEmpty())
                        <div class="bg-white p-12 rounded-3xl text-center text-gray-400 shadow-xl border border-gray-100">
                            ...
                            <p class="mt-2 text-sm">{{ __('ចាប់ផ្តើមដោយបន្ថែមដេប៉ាតឺម៉ង់ដំបូងរបស់អ្នកដើម្បីគ្រប់គ្រងកម្មវិធីសិក្សា។') }}</p>
                        </div>
                    @else
                            ...
                            @foreach ($departments as $department)
                                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 flex flex-col justify-between hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                    ...
                                    <div class="flex justify-end space-x-3 mt-auto">
                                        ...
                                        {{-- <a href="{{ route('professor.edit-department', $department->id) }}" class="p-3 bg-gray-100 rounded-full text-yellow-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('កែប្រែ') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                                <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('professor.delete-department', $department->id) }}" method="POST" onsubmit="return confirm('{{ __('តើអ្នកពិតជាចង់លុបដេប៉ាតឺម៉ង់នេះមែនទេ? វានឹងលុបកម្មវិធីសិក្សាដែលពាក់ព័ន្ធទាំងអស់។') }}')">
                                            @csrf
                                            @method('DELETE')
