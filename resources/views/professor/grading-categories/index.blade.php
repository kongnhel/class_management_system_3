        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl p-8 border border-gray-100">
                    <h4 class="text-2xl font-bold text-gray-700 mb-6">{{ __('create_new_category') }}</h4>
                    <form action="{{ route('professor.grading-categories.store', $course->id) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label for="name_km" class="block text-sm font-medium text-gray-700">{{ __('category_name_khmer') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="name_en" class="block text-sm font-medium text-gray-700">{{ __('category_name_english') }}</label>
                        ...
                        <div>
                            <label for="weight_percentage" class="block text-sm font-medium text-gray-700">{{ __('weight_percentage') }} <span class="text-red-500">*</span></label>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl p-8 border border-gray-100">
                    <h4 class="text-2xl font-bold text-gray-700 mb-6">{{ __('existing_categories') }}</h4>
                    @php
                        $totalWeight = $course->gradingCategories->sum('weight_percentage');
                    @endphp
                    <div class="mb-4">
                        <div class="flex justify-between mb-1">
                            <span class="text-base font-medium text-emerald-700">{{ __('total_percentage') }}</span>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2.5">
                         ...
                         @if($totalWeight < 100)
                             <p class="text-sm text-yellow-600 mt-2">{{ __('remaining') }} {{ 100 - $totalWeight }}% {{ __('more_to_complete') }}</p>
                        @elseif($totalWeight > 100)
                            <p class="text-sm text-red-600 mt-2">{{ __('total_percentage_exceeds_100') }}</p>
                        @else
                             <p class="text-sm text-green-600 mt-2">{{ __('total_percentage_is_100') }}</p>
                        @endif
                    </div>
                    <div class="space-y-3">
                        @forelse ($course->gradingCategories as $category)
                            <div class="flex items-center justify-between p-4 rounded-2xl bg-gray-50 border">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <form action="{{ route('professor.grading-categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('{{ __('confirm_delete_category') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <p class="text-center text-gray-500 py-8">{{ __('no_grading_categories_created_yet') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
